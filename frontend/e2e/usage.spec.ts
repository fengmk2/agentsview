...
test.describe("Usage page", () => {
  ...
  test("shows attribution panel with treemap", async ({ page }) => {
    ...
    await expect(page.locator(".treemap-container svg")).toBeVisible();
  });
  ...
  test("filter dropdown opens and shows items", async ({ page }) => {
    ...
    const trigger = page.locator(".filter-dropdown .filter-trigger").first();
    await trigger.click();

    // Dropdown panel should appear with rows.
    await expect(page.locator(".dropdown-panel").first()).toBeVisible();
    await expect(page.locator(".dropdown-row").first()).toBeVisible();
  });
  ...
  test("excluding a project updates total cost", async ({ page }) => {
    ...
    const trigger = page.locator(".filter-dropdown .filter-trigger").first();
    await trigger.click();
    await page.locator(".dropdown-row").filter({ hasText: "project-delta" }).first().click();
  ...
  test("select all / deselect all buttons work", async ({ page }) => {
    ...
    // Click "Deselect all".
    await page.locator(".bulk-btn").filter({ hasText: "Deselect all" }).first().click();

    // Trigger label should show "None".
    await expect(trigger).toContainText("None");

    // Click "Select all".
    await page.locator(".bulk-btn").filter({ hasText: "Select all" }).first().click();

    // Trigger label should show "All".
    await expect(trigger).toContainText("All");
  });

  test("top nav shows Usage button as active", async ({ page }) => {
    const usageBtn = page.locator('.nav-btn[aria-label="Usage"]');
